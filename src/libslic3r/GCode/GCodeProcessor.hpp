...
namespace Slic3r {
    ...
    public:
        ...
        // Update an ASCII G-code loaded by process_file() in place the way a sliced print is post-processed: M73 remaining time
        // lines, estimated printing time and filament statistics. Meant for the G-codes generated by PrusaSlicer without slicing,
        // e.g. the calibration patterns. The G-code stays ASCII. The printing time is estimated with klipper_estimator_limits
        // if they are valid, as they are not stored in the G-code.
        void post_process_file(const std::string& klipper_estimator_limits);
